/**
 * Fondation Botnar Dashboard - Chart.js Configuration
 * 
 * This file provides the base Chart.js configuration with Botnar theming,
 * accessibility features, and performance optimizations for dashboard use.
 */
...
// Import date adapter for time series charts
 ...
 * Extracted from Tailwind configuration for Chart.js use
 */
export const BotnarColors = {
 ...
 * Provides consistent theming and accessibility across all charts
 */
export const BotnarChartDefaults = {
  responsive: true,
  ...
  plugins: {
    ...
    tooltip: {
      ...
      titleColor: BotnarColors.text.primary,
      bodyColor: BotnarColors.text.secondary,
      borderColor: BotnarColors.border.light,
      borderWidth: 1,
      cornerRadius: 8,
      padding: 12,
      displayColors: true,
      ...
      callbacks: {
        ...
        afterLabel: function(context) {
          // Announce to screen readers (handled by Alpine.js components)
          return '';
        }
      }
    },
  ...
  scales: {
    x: {
      grid: {
        display: false,
        color: BotnarColors.border.light
      },
      ticks: {
        color: BotnarColors.text.secondary,
        font: {
          size: 12,
...
export const BarChartConfig = {
  ...
  options: {
    ...BotnarChartDefaults,
    scales: {
      ...BotnarChartDefaults.scales,
      x: {
        ...BotnarChartDefaults.scales.x,
  ...
  options: {
    ...
    elements: {
      ...
      point: {
        ...
        hoverRadius: 6,
        borderWidth: 2,
        backgroundColor: BotnarColors.background.primary
      }
    },
    scales: {
      ...BotnarChartDefaults.scales,
...
export const PieChartConfig = {
  ...
  options: {
    ...BotnarChartDefaults,
    plugins: {
      ...BotnarChartDefaults.plugins,
      tooltip: {
        ...BotnarChartDefaults.plugins.tooltip,
        callbacks: {
          label: function(context) {
            const dataset = context.dataset;
    ...
    elements: {
      arc: {
        borderWidth: 2,
        borderColor: BotnarColors.background.primary
      }
    }
  }
};
...
export function createDataset(label, data, options = {}) {
  const defaults = {
    label: label,
    data: data,
    backgroundColor: BotnarColors.categorical[0],
    borderColor: BotnarColors.categorical[0],
    borderWidth: 2,
    ...options
  };
...
export function createMultipleDatasets(datasets) {
  return datasets.map((dataset, index) => {
    const colorIndex = index % BotnarColors.categorical.length;
    return createDataset(dataset.label, dataset.data, {
      backgroundColor: BotnarColors.categorical[colorIndex],
      borderColor: BotnarColors.categorical[colorIndex],
      ...dataset.options
    });
  });
}

// Create gradient backgrounds for area charts
export function createGradient(canvas, color, opacity = 0.1) {
...
// Accessibility helper: Generate data table structure
export function generateDataTable(chart) {
  const { data } = chart;
  const table = {
    headers: ['Category', ...data.datasets.map(d => d.label)],
    rows: []
  };
  ...
  data.labels.forEach((label, index) => {
    ...
    data.datasets.forEach(dataset => {
      row.push(formatValue(dataset.data[index]));
    });
    table.rows.push(row);
  });
...
export function measureChartPerformance(chartInstance, chartType) {
  ...
  window.chartPerformance.push({
    ...
    dataPoints: chartInstance.data.datasets.reduce((total, dataset) => total + dataset.data.length, 0)
  });
...
/**
 * Chart.js Plugin for Enhanced Accessibility
 */
...
function navigateDataPoints(chart, direction) {
  ...
  announceToScreen(`${label}: ${formatValue(value)}`);
}
...
function generateChartDescription(chart) {
  ...
  if (data.datasets.length > 0) {
    ...
    description += `Total value: ${formatValue(total)}. `;
  }
...
function announceToScreen(text) {
...
// Export Chart.js for global use
export { Chart };
